      ...
      <div className="card card-pad flex flex-wrap gap-3 items-end">
        <div className="flex-1 min-w-[220px]">
          ...
          <div className="relative">
            ...
            <input className="input pl-9" placeholder="Search domain..." value={q} onChange={(e) => setQ(e.target.value)} />
          </div>
        </div>
        {accounts.length > 0 && (
          <div className="min-w-[180px]">
            <label className="label">Account</label>
            <select className="input" value={acc} onChange={(e) => setAcc(e.target.value ? Number(e.target.value) : "")}>
      ...
      <div className="card overflow-hidden">
        {list === null ? (
          ...
          <ul className="divide-y divide-ink-800">
            {list.map((d) => (
              <li key={d.id}>
                <Link to={`/domains/${d.id}`} className="flex items-center justify-between px-5 py-3 hover:bg-ink-900">
                  ...
                  <div className="flex items-center gap-2">
                    {d.status === "active" ? <span className="badge-green">active</span> :
                      d.status ? <span className="badge-amber">{d.status}</span> : null}
                  </div>
                </Link>
              </li>
            ))}
