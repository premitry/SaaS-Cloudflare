            ...
            <tbody>
              {list.map((a) => (
                <tr key={a.id}>
                  ...
                  <td className="font-mono text-xs text-ink-400">{a.account_id ?? "-"}</td>
                  <td>
                  ...
                  <td className="text-right">
                    <div className="inline-flex items-center gap-1">
                      <button className="btn-secondary" onClick={() => onSync(a.id)} disabled={busy === a.id}>
                        <IconRefresh size={14} />
                        Sync
                      </button>
                      <button className="btn-danger" onClick={() => onDelete(a.id)} disabled={busy === a.id}>
                        <IconTrash size={14} />
                      </button>
                    </div>
                  </td>
      ...
      <Modal
        ...
        footer={
          <>
            ...
            <button className="btn-primary" onClick={onCreate} disabled={submitting || !name || !token}>
      ...
      >
        <div className="space-y-3">
          <div>
            ...
            <input className="input" value={name} onChange={(e) => setName(e.target.value)} placeholder="Company A" />
          </div>
          <div>
            <label className="label">Email (optional)</label>
            <input className="input" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="ops@company.com" />
          </div>
          <div>
