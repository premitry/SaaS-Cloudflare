    ...
    (async () => {
      try {
        const [o, c] = await Promise.all([
          api.get<Overview>(`/api/domains/${id}/overview`),
          api.get<SetupCheck>(`/api/domains/${id}/setup-check`).catch(() => null),
        ]);
        setOv(o);
        setCheck(c);
      } catch (e) {
  ...
  return (
    <div className="space-y-5">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <Stat label="Email Routing" value={ov.email_routing ? (ov.email_routing.enabled || ov.email_routing.status === "ready" ? "Enabled" : "Disabled") : "-"} />
      ...
      {ov.zone?.name_servers && ov.zone.name_servers.length > 0 && (
        <div className="card card-pad">
          ...
          <ul className="font-mono text-sm text-ink-300 space-y-1">
            {ov.zone.name_servers.map((n) => <li key={n}>{n}</li>)}
          </ul>
        </div>
      )}
...
function CheckCard({ title, items }: { title: string; items: Array<{ label: string; ok: boolean }> }) {
  return (
    <div className="card card-pad">
      ...
      <ul className="space-y-1.5 text-sm">
        {items.map((it) => (
          <li key={it.label} className="flex items-center gap-2">
            {it.ok ? <span className="text-emerald-400"><IconCheck size={14} /></span> :
              <span className="text-amber-400"><IconWarn size={14} /></span>}
            <span className={it.ok ? "text-ink-200" : "text-ink-300"}>{it.label}</span>
          </li>
        ))}
      </ul>
    </div>
