...
export default function AdminLoginPage() {
  ...
  // Run the diagnostics check once so we can warn the user if the worker is
  // misconfigured before they even try to log in.
  useEffect(() => {
    diag
      .check()
      .then((d) => {
        ...
        if (!d.db_ready) {
          setHint(
            d.db_error
              ? `Database not ready: ${d.db_error}. Try: npm run db:migrate:local`
              : "Database not initialised yet. Run `npm run db:migrate:local`."
          );
        } else if (!d.has_jwt_secret) {
          setHint(
            "JWT_SECRET is not set. Run `npm run setup` again to regenerate worker/.dev.vars."
          );
        }
      })
      .catch((e) => {
        setHint(
          `Cannot reach the worker API. ${formatError(e)} ` +
            "Run `npm run dev:worker` in another terminal."
        );
      });
  }, []);
      ...
      <div className="w-full max-w-sm">
        ...
        <div className="card card-pad space-y-4">
          ...
          <form onSubmit={onSubmit} className="space-y-3">
            ...
            >
              {loading ? "Signing in..." : isFresh ? "Create admin & sign in" : "Sign in"}
            </button>
          </form>
