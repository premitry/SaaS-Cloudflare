...
export default function DnsPage({ params }: { params: { id: string } }) {
  ...
  async function load() {
    try {
      const r = await api.get<DnsRecord[]>(`/api/domains/${params.id}/dns`);
      setList(r);
    } catch (e) {
  ...
  async function onDelete(r: DnsRecord) {
    ...
    try {
      await api.del(`/api/domains/${params.id}/dns/${r.id}`);
  ...
  async function onToggleProxy(r: DnsRecord, v: boolean) {
    try {
      await api.put(`/api/domains/${params.id}/dns/${r.id}`, {
        type: r.type,
        name: r.name,
        content: r.content,
        ttl: r.ttl,
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div className="relative w-full sm:w-72">
          ...
          <input
            ...
            placeholder="Filter name, content, type..."
            value={q}
      ...
      <div className="card overflow-hidden">
        {list === null ? (
          ...
          <div className="p-8 text-center text-ink-400 text-sm">No records.</div>
        ) : (
          <table className="table">
            ...
            <tbody>
              {filtered.map((r) => (
                <tr key={r.id}>
                  <td>
                    <span className="badge-gray font-mono">{r.type}</span>
                  </td>
                  <td className="font-mono text-ink-100">{r.name}</td>
                  <td className="font-mono text-ink-300 break-all max-w-md">
                    {r.content}
                  </td>
