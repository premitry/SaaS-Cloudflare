...
export default function EmailRoutingPage({
  ...
  async function load() {
    try {
      setData(await api.get<Bundle>(`/api/domains/${params.id}/email-routing`));
    } catch (e) {
  ...
  async function onToggleRouting(v: boolean) {
    try {
      await api.post(
        `/api/domains/${params.id}/email-routing/${v ? "enable" : "disable"}`
      );
  ...
  async function onCatchAll(enabled: boolean, forwardTo?: string) {
    try {
      await api.put(`/api/domains/${params.id}/email-routing/catch-all`, {
        enabled,
  ...
  async function onDeleteRule(r: Rule) {
    ...
    try {
      await api.del(`/api/domains/${params.id}/email-routing/rules/${r.id}`);
  ...
  async function onToggleRule(r: Rule, v: boolean) {
    try {
      await api.put(`/api/domains/${params.id}/email-routing/rules/${r.id}`, {
        ...r,
        enabled: v,
      });
  ...
  const enabled =
    !!data.settings?.enabled || data.settings?.status === "ready";
  const caEnabled = !!data.catch_all?.enabled;
  const caForward =
    data.catch_all?.actions?.find((a) => a.type === "forward")?.value?.[0] ??
    "";

  return (
    <div className="space-y-4">
      {/* Routing toggle + Catch-all */}
      ...
      {enabled && (
        <div className="card overflow-hidden">
          ...
          {data.rules.length === 0 ? (
            <div className="p-8 text-center text-ink-400 text-sm">No rules yet.</div>
          ) : (
            <table className="table">
              ...
              <tbody>
                {data.rules.map((r) => {
                  const match =
                    r.matchers?.[0]?.value ??
                    (r.matchers?.[0]?.type === "all" ? "* (all)" : "-");
                  const forward =
                    r.actions?.find((a) => a.type === "forward")?.value ?? [];
                  return (
                    <tr key={r.id}>
...
function CatchAllRow({
  ...
  enabled: boolean;
  forwardTo: string;
  onSave: (enabled: boolean, forwardTo?: string) => void;
}) {
    ...
    <div className="rounded-lg border border-ink-700 bg-ink-850 p-3">
      <div className="flex items-center justify-between">
        <div>
          ...
          <div className="text-xs text-ink-400 mt-0.5">
            Forward any unmatched address ({"*@domain"}) to a destination.
          </div>
        </div>
        <Toggle
          checked={enabled}
          onChange={(v) => onSave(v, val || undefined)}
        />
      </div>
      {enabled && (
...
function RuleForm({
  ...
  async function onSubmit() {
    setSubmitting(true);
    try {
      await api.post(`/api/domains/${domainId}/email-routing/rules`, {
        match,
        forward_to: forward,
      });
      ...
      footer={
        <>
          ...
          <button
            className="btn-primary"
            onClick={onSubmit}
            disabled={submitting || !match || !forward}
          >
      ...
      <div className="space-y-3">
        ...
        <div>
          ...
          <p className="text-xs text-ink-500 mt-1">
            The destination must be verified in Cloudflare Email Routing.
          </p>
        </div>
      </div>
    </Modal>
