  ...
  async function load() {
    try {
      ...
      const [u, a] = await Promise.all([
        ...
        accounts.length === 0 ? api.get<CfAccount[]>("/api/cf-accounts") : Promise.resolve(accounts),
      ]);
      setUsers(u);
    ...
    <div className="p-6 space-y-5 max-w-7xl">
      ...
      <div className="card overflow-hidden">
        {users === null ? (
          ...
          <div className="p-8 text-center text-ink-400 text-sm">No users found.</div>
        ) : (
          <table className="table">
            ...
            <tbody>
              {users.map((u) => {
                ...
                const accessLabel =
                  !perms ? "-" :
                  perms.can_full_access
                    ? "Full"
                    : [
                        perms.can_dns ? "DNS" : null,
                        perms.can_email ? "Email" : null,
                        perms.can_domain_settings ? "Settings" : null,
                      ]
                        .filter(Boolean)
                        .join(" + ") || "None";
                return (
                  <tr key={u.id}>
                    ...
                    <td>
                      <span className="badge-blue">{u.domain_count}</span>
                    </td>
                    <td className="text-ink-200 text-xs">{accessLabel}</td>
                    <td className="text-ink-400 text-xs">
                      {u.is_permanent ? (
                        <span className="badge-gray">permanent</span>
                      ) : (
                        formatDate(u.expired_at)
                      )}
                    </td>
                    <td>
                      {u.status === "active" ? (
                    ...
                    <td className="text-right">
                      <div className="inline-flex items-center gap-1">
                        ...
                        <button className="btn-secondary" onClick={() => onRegen(u)}>
                          <IconRefresh size={14} />
                        </button>
...
function UserForm({
  ...
  const [pSettings, setPSettings] = useState<boolean>(
    !!editing?.permissions?.can_domain_settings
  );
  const [pFull, setPFull] = useState<boolean>(
    !!editing?.permissions?.can_full_access
  );
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([all, detail]) => {
        setAllDomains(all);
        if (detail) {
          setSelectedDomains(new Set(detail.domains.map((d) => d.id)));
        }
      })
      .catch((e) =>
        toast.error(e instanceof ApiError ? e.message : "Load domains failed")
      )
  ...
  async function onSubmit() {
    ...
    try {
      ...
      if (!permanent) {
        if (duration === "custom") {
          payload.expired_at = expired || null;
        } else {
      ...
      <div className="space-y-5">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <div>
            ...
            <select
              ...
              value={cfId}
              disabled={!!editing}
              onChange={(e) => setCfId(e.target.value ? Number(e.target.value) : "")}
            >
              {accounts.map((a) => (
                <option key={a.id} value={a.id}>
                  {a.name}
        ...
        <fieldset className="space-y-2">
          ...
          <div className="flex flex-wrap gap-2">
            <Choice active={permanent} onClick={() => setPermanent(true)}>
              Permanent
            </Choice>
            <Choice
              active={!permanent && duration === 1}
              onClick={() => {
                setPermanent(false);
                setDuration(1);
              }}
            ...
            <Choice
              active={!permanent && duration === "custom"}
              onClick={() => {
                setPermanent(false);
                setDuration("custom");
              }}
        ...
        <fieldset>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            ...
            <PermRow
              ...
              onChange={setPDns}
              title="DNS Access"
              desc="Manage DNS records (add/edit/delete, proxy toggle)"
            />
            <PermRow
              checked={pFull || pEmail}
              disabled={pFull}
              onChange={setPEmail}
              title="Email Routing"
              desc="Manage forwarding rules and catch-all"
            />
            <PermRow
              checked={pFull || pSettings}
              disabled={pFull}
              onChange={setPSettings}
              title="Domain Settings"
              desc="SSL mode, Always HTTPS, cache purge"
            />
          </div>
        </fieldset>
        ...
        <fieldset>
          ...
          <div className="card border-ink-700">
            <div className="p-3 border-b border-ink-800 flex items-center gap-2">
              ...
              <input
                className="bg-transparent flex-1 text-sm placeholder:text-ink-500 focus:outline-none"
            ...
            <div className="max-h-56 overflow-y-auto divide-y divide-ink-800">
              {loadingDomains ? (
                ...
                <div className="p-4 text-sm text-ink-500">No domains.</div>
              ) : (
                filteredDomains.map((d) => {
                  ...
                  return (
                    <label
                      key={d.id}
                      className="flex items-center gap-3 px-3 py-2 hover:bg-ink-800 cursor-pointer"
                    >
                      <input
                        type="checkbox"
        ...
        {!editing && (
          <div>
            ...
            <input
              ...
              placeholder="USER, DNS, VIP..."
            />
            <p className="text-xs text-ink-500 mt-1">
              The generated code will be {`<PREFIX>-XXXXX`}.
            </p>
          </div>
        )}
      </div>
