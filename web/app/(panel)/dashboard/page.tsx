...
import { IconCloud, IconGlobe, IconList, IconUsers } from "../../../components/Icon";

type CfAccount = { id: number; name: string; domain_count: number };
type Domain = { id: number; domain: string; cf_account_id: number; status?: string };
        ...
        <>
          ...
          <section className="card">
            <header className="flex items-center justify-between px-5 py-3 border-b border-ink-700">
              ...
              <Link href="/domains" className="text-sm text-accent-300 hover:text-accent-200">
                View all
              </Link>
            </header>
            {domains.length === 0 ? (
              ...
              <ul className="divide-y divide-ink-800">
                {domains.slice(0, 8).map((d) => (
                  <li key={d.id} className="flex items-center justify-between px-5 py-3">
                    <Link href={`/domains/${d.id}`} className="text-ink-100 hover:text-accent-300">
                      {d.domain}
                    </Link>
                    {d.status ? (
                      <span
                        className={
                          d.status === "active" ? "badge-green" : "badge-amber"
                        }
                      >
                        {d.status}
                      </span>
          ...
          {actor?.type === "admin" && audits.length > 0 && (
            <section className="card">
              <header className="flex items-center justify-between px-5 py-3 border-b border-ink-700">
                ...
                <Link href="/audit-logs" className="text-sm text-accent-300 hover:text-accent-200">
                  View all
                </Link>
              </header>
              <ul className="divide-y divide-ink-800">
...
function StatCard({
  ...
  return (
    <Link href={href} className="card card-pad hover:border-ink-600 transition block">
